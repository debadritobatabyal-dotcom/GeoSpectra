...
round_trip_test.py
==================
Comprehensive round-trip verification for the manganese prospectivity model.

Tests:
1. POSITIVE rows from CSV → prediction pipeline → verify probabilities
2. UNLABELLED rows from CSV → prediction pipeline → verify probabilities
3. Feature schema validation
4. Out-of-domain detection
5. Missing feature reporting
"""
...
def main():
    ...
    # Filter POSITIVE and UNLABELLED
    ...
    for idx, row in pos_df.iterrows():
        # Use complete original feature dictionary
    ...
    # Test with sample of UNLABELLED rows (all would be too many)
    print("\n" + "=" * 80)
    ...
    # Test website feature pipeline (clicked coordinate → feature extraction → prediction)
    print("\n" + "=" * 80)
    print("   WEBSITE FEATURE PIPELINE TEST")
    print("=" * 80)
    
    # Simulate what the website does: click coordinate → get satellite features → predict
    test_coords = [
